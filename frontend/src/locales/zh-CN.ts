...
export default {
  ...
  failover: {
    ...
    enabled: '已启用',
    disabled: '已禁用',
    enableDescription: '启用后，只有当连续错误次数达到配置的阈值时才会触发故障转移。禁用时，任何符合条件的错误都会立即触发故障转移。',
    rulesSection: '错误阈值规则',
    rulesDescription: '定义每种错误类型允许的连续错误次数，超过后触发故障转移到下一个渠道/密钥。',
    errorCodes: '错误码',
    threshold: '阈值',
    codesPlaceholder: '如 401,403 或 others',
    addRule: '添加规则',
    howItWorks: '工作原理：',
    howItWorksItem1: '错误码可用逗号分隔（如 "401,403"），"others" 表示未匹配的其他错误码',
    howItWorksItem2: '阈值 = 触发故障转移所需的连续错误次数（1 = 立即触发）',
    howItWorksItem3: '成功响应（2xx）后计数器重置',
    reset: '恢复默认',
    ...
    // 错误码参考提示
    errorCodeReference: '常见 HTTP 错误码',

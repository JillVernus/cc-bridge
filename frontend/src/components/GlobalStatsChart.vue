...
import { api, type StatsHistoryResponse, type Duration } from '../services/api'
...
// Summary data
const summary = computed(() => historyData.value?.summary || null)
...
const refreshData = async (isAutoRefresh = false) => {
  ...
  try {
    const endpoint = props.apiType === 'messages' ? '/v1/messages' : '/v1/responses'
    const newData = await api.getStatsHistory(selectedDuration.value, endpoint)

...
func (s *Server) finalizeInterceptedCompletionRecord(pendingLogID, hostOnly string, window xInitiatorCostWindowRef, record *requestlog.RequestLog) {
	if s == nil || record == nil {
		return
	}
	...
	s.applyWindowedCostCompletion(hostOnly, window, record.CompleteTime, record.Price)
}
